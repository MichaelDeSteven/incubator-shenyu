...
public class BrpcPlugin extends AbstractShenyuPlugin {
    ...
    protected Mono<Void> doExecute(final ServerWebExchange exchange, final ShenyuPluginChain chain, final SelectorData selector, final RuleData rule) {
        ...
        BrpcInvokePrxList brpcProxyList = ApplicationConfigCache.getInstance().get(metaData.getPath());
        Object proxy = brpcProxyList.getBrpcInvokePrxList().get(0).getInvokePrx();
        Method method = brpcProxyList.getMethod();
        CompletableFuture future;
        try {
            future = (CompletableFuture) method
                    .invoke(proxy, ProxyInfoUtil.getParamArray(brpcProxyList.getParamTypes(), brpcProxyList.getParamNames(), body));
        } catch (Exception e) {
            LOG.error("Invoke brpc error", e);
            exchange.getResponse().setStatusCode(HttpStatus.INTERNAL_SERVER_ERROR);
            Object error = ShenyuResultWrap.error(exchange, ShenyuResultEnum.BRPC_INVOKE);
            return WebFluxResultUtils.result(exchange, error);
        }
        return Mono.fromFuture(future.thenApply(ret -> {
            ...
            return ret;
        })).onErrorMap(m -> new ShenyuException("failed to invoke brpc")).then(chain.execute(exchange));
    }
