...
/**
 * The Brpc ServiceBean PostProcessor.
 */
public class BrpcServiceBeanPostProcessor implements BeanPostProcessor {
    ...
    public BrpcServiceBeanPostProcessor(final PropertiesConfig clientConfig, final ShenyuClientRegisterRepository shenyuClientRegisterRepository) {
    ...
    public Object postProcessAfterInitialization(final Object bean, final String beanName) throws BeansException {
        if (AnnotationUtils.findAnnotation(bean.getClass(), ShenyuBrpcService.class) != null) {
            handler(bean);
        }
        return bean;
    }
    ...
    private void handler(final Object serviceBean) {
        ...
        String serviceName = clazz.getAnnotation(ShenyuBrpcService.class).serviceName();
        for (Method method : methods) {
            ShenyuBrpcClient shenyuBrpcClient = method.getAnnotation(ShenyuBrpcClient.class);
            if (Objects.nonNull(shenyuBrpcClient)) {
                publisher.publishEvent(buildMetaDataDTO(serviceName, shenyuBrpcClient, method, buildRpcExtJson(method)));
            }
        }
    }

    private MetaDataRegisterDTO buildMetaDataDTO(final String serviceName, final ShenyuBrpcClient shenyuBrpcClient, final Method method, final String rpcExt) {
        String ipAndPort = this.ipAndPort;
        String path = this.contextPath + shenyuBrpcClient.path();
    ...
    private BrpcRpcExt.RpcExt buildRpcExt(final Method method) {
        ...
        return new BrpcRpcExt.RpcExt(method.getName(), params, method.getReturnType().getName());
    }
